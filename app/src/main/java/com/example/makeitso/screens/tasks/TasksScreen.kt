...
fun TasksScreen(
  ...
  // Obtener el estado de las tareas
  val tasks = viewModel
    .tasks
  ...
  TasksScreenContent(
    ...
    tasks = tasks.value // Pasar las tareas al contenido
  )
...
fun TasksScreenContent(
  ...
  openScreen: (String) -> Unit,
  tasks: List<Task> // Agregar parámetro para las tareas
) {
  Scaffold(
    ...
    Column(modifier = Modifier.fillMaxWidth().fillMaxHeight()) {
      ...
      LazyColumn {
        items(tasks, key = { it.id }) { taskItem -> // Actualizar para usar las tareas
          TaskItem(
            task = taskItem,
            options = listOf(),
...
fun TasksScreenPreview() {
  MakeItSoTheme {
    TasksScreenContent(
      ...
      openScreen = { },
      tasks = emptyList() // Agregar un valor predeterminado para las tareas
    )
  }
}